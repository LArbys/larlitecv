...
namespace larlitecv {
  ...
  public:
    ...
    bool flashMatchTrackEnds( const std::vector< larlite::event_opflash* >& opflashsets, const std::vector<larcv::Image2D>& tpc_imgs,
			      ...
			      std::vector< BoundarySpacePoint >& trackendpts, std::vector< int >& endpoint_flash_idx, std::vector< int >& endpoint_boundary_type_idx ); // The last two arguments were added to find out which flash is matched to which endpoint.
