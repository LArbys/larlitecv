...
namespace larlitecv {
  ...
  const std::vector<larcv::Pixel2DCluster>& BMTrackCluster3D::getTrackPixelsFromImages( const std::vector<larcv::Image2D>& imgs, const std::vector<larcv::Image2D>& badchimgs,
											const std::vector<float>& thresholds, const std::vector<int>& neighborhood_size, 
    ...
    plane_pixels.resize(nplanes);
    
    for ( int p=0; p<nplanes; p++ ) {
      for ( auto const& pix : planepixs[p] ) {
	larcv::Pixel2D pixel( pix[0], pix[1] );
	plane_pixels.at(p).emplace_back( std::move(pixel) );
      }
    }
