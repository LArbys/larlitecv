...
namespace larlitecv {
			...
			for (size_t itrack_b=itrack_a+1; itrack_b<ntracks; itrack_b++ ) {
				...
				if ( start_dist<20.0 && end_dist<20.0 ) {
				  ...
				  if (pair_cosine > 0.8) {
				    ...
				    std::vector < std::vector <double> > endpoint_coordinates_of_new_track = longerTrackExtensionCoordinates(longer_line._pt1, longer_line._pt2, shorter_lineseg._start, shorter_lineseg._end);
  ...
  bool pointProximity(geoalgo::Line long_line, geoalgo::LineSegment* short_linesegment, int resolution, double maximum_dist) {
    ...
    // line.
    int    line_longest_dim_idx              = longestDimFinder(long_line._pt1, long_line._pt2);
    int    short_linesegment_longest_dim_idx = longestDimFinder(short_linesegment._start, short_linesegment._end);
    ...
    std::vector < std::vector<double> > long_line_points           = getTrackPoints(long_line._pt1, long_line._pt2, resolution, line_shorter_dim_idx_vector, line_longest_dim_idx);
    std::vector < std::vector<double> > short_linesegment_points   = getTrackPoints(short_linesegment._start, short_linesegment._end, resolution, short_linesegment_shorter_dim_idx_vector, short_linesegment_longest_dim_idx); 
	...
	if (sqrt((long_line_points[0] - short_linesegment_points[0]) * (long_line_points[0] - short_linesegment_points[0]) + (long_line_points[1] - short_linesegment_points[1]) * (long_line_points[1] - short_linesegment_points[1]) + (long_line_points[2] - short_linesegment_points[2]) * (long_line_points[2] - short_linesegment_points[2])) < maximum_dist) {
  ...
  std::vector < std::vector<double> > getTrackPoints(const std::vector<double> point_one_coords, const std::vector<double> point_two_coords, int resolution, std::vector <int> shorter_dim_idx_vector, int longest_dim_idx) {
    ...
    // Divide the difference between 'largest_spread_var_first' and 'largest_spread_var_second' into 'resolution' number of equal segments, and then find which of the 'largest_spread' variables is greatest.
    ...
    for (size_t line_iter = 0; line_iter < resolution + 1; line_iter++) {
      ...
      largest_spread_coord_values[line_iter]  = current_value_largest_spread_coord;
      shorter_spread_coord1_values[line_iter] = current_value_shorter_spread_coord1;
      shorter_spread_coord2_values[line_iter] = current_value_shorter_spread_coord2;
	...
	// Append the variables onto the 'line_coord_values' line in the following order: [largest_spread_coord_values[coord_iter], shorter_spread_coord1_values[coord_iter], shorter_spread_coord2_values[coord_iter]].
	line_coord_values.push_back([largest_spread_coord_values[coord_iter], shorter_spread_coord1_values[coord_iter], shorter_spread_coord2_values[coord_iter]]);

      }

      // longest dimension - x, shorter dimension #1 - z, shorter dimension #2 - y
      if (longest_dim_idx == 0 && shorter_dim_idx_vector[0] == 2 && shorter_dim_idx_vector[1] == 1) {

	// Append the variables onto the 'line_coord_values' line in the following order: [largest_spread_coord_values[coord_iter], shorter_spread_coord2_values[coord_iter], shorter_spread_coord1_values[coord_iter]].
	line_coord_values.push_back([largest_spread_coord_values[coord_iter], shorter_spread_coord2_values[coord_iter], shorter_spread_coord1_values[coord_iter]]);

      }

      // longest dimension - y, shorter dimension #1 - x, shorter dimension #2 - z
      if (longest_dim_idx == 1 && shorter_dim_idx_vector[0] == 0 && shorter_dim_idx_vector[1] == 2) {

	// Append the variables onto the 'line_coord_values' line in the following order: [shorter_spread_coord1_values[coord_iter], largest_spread_coord_values[coord_iter], shorter_spread_coord2_values[coord_iter]].
	line_coord_values.push_back([shorter_spread_coord1_values[coord_iter], largest_spread_coord_values[coord_iter], shorter_spread_coord2_values[coord_iter]]);

      }

      // longest dimension - y, shorter dimension #1 - z, shorter dimension #2 - x
      if (longest_dim_idx == 1 && shorter_dim_idx_vector[0] == 2 && shorter_dim_idx_vector[1] == 0) {

	// Append the variables onto the 'line_coord_values' line in the following order: [shorter_spread_coord2_values[coord_iter], largest_spread_coord_values[coord_iter], shorter_spread_coord1_values[coord_iter]].
	line_coord_values.push_back([shorter_spread_coord2_values[coord_iter], largest_spread_coord_values[coord_iter], shorter_spread_coord1_values[coord_iter]]);

      }

      // longest dimension - z, shorter dimension #1 - x, shorter dimension #2 - y
      if (longest_dim_idx == 2 && shorter_dim_idx_vector[0] == 0 && shorter_dim_idx_vector[1] == 1) {

	// Append the variables onto the 'line_coord_values' line in the following order: [shorter_spread_coord1_values[coord_iter], shorter_spread_coord2_values[coord_iter], largest_spread_coord_values[coord_iter]].
	line_coord_values.push_back([shorter_spread_coord1_values[coord_iter], shorter_spread_coord2_values[coord_iter], largest_spread_coord_values[coord_iter]]);

      }

      // longest dimension - z, shorter dimension #1 - y, shorter dimension #2 - x
      if (longest_dim_idx == 2 && shorter_dim_idx_vector[0] == 1 && shorter_dim_idx_vector[1] == 0) {

	// Append the variables onto the 'line_coord_values' line in the following order: [shorter_spread_coor2_values[coord_iter], shorter_spread_coord1_values[coord_iter], largest_spread_coord_values[coord_iter]].
	line_coord_values.push_back([shorter_spread_coord2_values[coord_iter], shorter_spread_coord1_values[coord_iter], largest_spread_coord_values[coord_iter]]);
  ...
  std::vector <std::vector <double> > longerTrackExtensionCoordinates(std::vector<double> longer_line_first_point_coords, std::vector<double> longer_line_second_point_coords, std::vector<double> shorter_linesegment_first_point_coords, std::vector<double> shorter_linesegment_second_point_coords) {
    ...
    if (shorter_linesegment_first_point_coord[2] > shorter_linesegment_second_point_coords[2])
    ...
    std::vector < vector<double> > output_coordinate_vector;
...
// 						   This function will be void, because it will just operate on the ‘tracks_v’ vector.
  void  concatenatingTwoTrackParts(std::vector< BMTrackCluster3D >& tracks_v, int itrack_a, int itrack_b, BMTrackCluster3D& track_a, BMTrackCluster3D& track_b, std::vector < std::vector <double> > endpoint_coordinates_of_new_track) {
    ...
    std::vector <double> shorter_lineseg_endpoint_coords = endpoint_coordinates_of_new_track[0];
    std::vector <double> longer_line_endpoint_coords     = endpoint_coordinates_of_new_track[1];
    ...
    std::vector <int> shorter_lineseg_index;
    std::vector <int> longer_line_index;
    ...
    std::vector <double> endpoint_x_coordinates = [shorter_lineseg_endpoint_coords[0], longer_line_endpoint_coords[0]];
    ...
    std::vector<int> shorter_lineseg_3Dpos;
    ...
    std::vector<int> longer_line_3Dpos;
    ...
    if (longer_line_indices[0] == 0) {
      ...
      shorter_lineseg_index       = track_b.track2d_index;
	...
	for (size_t plane_paths_iter = 0; plane_paths_iter < shorter_lineseg_plane_paths.size(); plane_paths_iter++) {
	  ...
	  tracks_v.at(itrack_a).path3d.push_back(shorter_lineseg_plane_paths[plane_paths_iter]);
	...
	for (size_t path3d_iter = 0; path3d_iter < shorter_lineseg_path3d.size(); path3d_iter++) {
      ...
      if (shorter_lineseg_endpoint_coords[1] > longer_line_endpont_coords[1]) {
	...
	// Append the coordinates of 'shorter_lineseg_plane_paths' one at a time to 'tracks_v.at(itrack_a).plane_paths' AT THE FRONT starting at t
	// he end of the list because the 'shorter_lineseg' is at a greater y-coordinate than the 'longer_line' is.                              
	for (size_t plane_paths_iter = (shorter_lineseg_plane_paths.size() - 1); plane_paths_iter > -1; plane_paths_iter--) {
	  
	  tracks_v.at(itrack_a).path3d.push_front(shorter_lineseg_plane_paths[plane_paths_iter]);

	}

	// Append the coordinates of 'shorter_lineseg_path3d' one at a time to 'tracks_v.at(itrack_a).path3D' AT THE FRONT starting at the 
	// end of the list, because the 'shorter_lineseg' is a greater y-coordinate than the 'longer_line' is.                            
	for (size_t path3d_iter = (shorter_lineseg_path3d.size() - 1); path3d_iter > -1; path3d_iter--) {
					  
	  tracks_v.at(itrack_a).path3d.push_front(shorter_lineseg_path3d[path3d_iter]);
      ...
      // I will include the information to continue on from the shorter linesegment outside the other loop in the same way
      // way as is done above with the track that is entirely contained by the outer track.
    ...
    if (longer_line_indices[0] == 1) {
      ...
      shorter_lineseg_index       = track_a.track2d_index;

      // 'longer_line'
      longer_line_plane_paths     = track_b.plane_paths;
      longer_line_path3d          = track_b.path3d;
      longer_line_index           = track_b.track2d_index;
	...
	tracks_v.at(itrack_b).track2d_index = longer_line_index;
	...
	for (size_t path3d_iter = 0; path3d_iter < shorter_lineseg_path3d.size(); path3d_iter++) {
	  ...
	  tracks_v.at(itrack_b).path_3d.push_back(shorter_lineseg_path3d[path3d_iter]);
	...
	tracks_v.at(itrack_b).start_index  = shorter_lineseg_index;
	...
	tracks_v.at(itrack_b).end_index    = longer_line_index;
	...
	// Append information from 'shorter_lineseg_plane_paths' onto the FRONT OF the 'tracks_v.at(itrack_b).plane_paths' 
	// vector, starting with the last entry (then the list will still be in order).    
	for (size_t plane_paths_iter = (shorter_lineseg_plane_paths.size() - 1); plane_paths_iter > -1; plane_paths_iter--) {
	...
	// Append information from 'shorter_line_seg_path3d' onto the FRONT OF the 'tracks_v.at(itrack_b).path3d' vector
