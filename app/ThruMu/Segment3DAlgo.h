...
namespace larlitecv {
  ...
  protected:
    ...
    std::vector< Segment2D_t > make2DSegments( const larcv::Image2D& img, const int lowrow, const std::vector<int>& hits_low,
					       const int highrow, const std::vector<int>& hits_high, const float threshold,
					       const int hit_width, const float frac_good );
    
    void checkSegmentCharge( const larcv::Image2D& img, const int low_row, const int low_col, const int high_row, const int high_col, const int hit_width,
			     const float threshold, int& nrows_w_charge, int& num_rows );

    void combine2Dinto3D( const std::vector< std::vector<Segment2D_t> >& plane_segments2d, const std::vector< larcv::Image2D >& img_v,
