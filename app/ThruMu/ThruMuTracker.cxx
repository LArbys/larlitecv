...
namespace larlitecv {
  ...
  void ThruMuTracker::flashMatchTracks( GeneralFlashMatchAlgoConfig& flash_match_config, const std::vector< const BoundarySpacePoint* >& spacepts,
    ...
    if ( anode_and_cathode_only ) {
      flashMatchAC( flash_match_config, qcluster_vector, boundary_flash_index_vector, track_endpoint_flash_v_from_pass,
		    track_endpoint_boundary_type_idx_v_from_pass, impossible_match_endpoint_v, track_endpoint_v_from_pass, well_matched_tracks_idx_v, already_matched_flash_v );
    }
    else {
      ...
      flashMatchYZFaceTracks( flash_match_config, trackclusters, qcluster_vector, boundary_flash_index_vector, impossible_match_endpoint_v,
			      track_endpoint_v_from_pass, well_matched_tracks_idx_v, already_matched_flash_v, entire_event );
  ...
  void ThruMuTracker::flashMatchYZFaceTracks( GeneralFlashMatchAlgoConfig& flash_match_config, const std::vector< larlitecv::BMTrackCluster3D >& trackclusters,
	...
	continue;
      ...
      rankTrackFlashMatches( flash_match_config, qclusters_being_checked, well_matched_tracks_idx_v,
			     *boundary_flash_index_vector.at( opflash_i ).popflash, boundary_flash_index_vector.at( opflash_i ).ivec, opflash_track_match_list );
