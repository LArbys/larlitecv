...
start_row  = meta.row( start_tick )
end_row    = meta.row( end_tick )
start_cols = std.vector("int")(3)
end_cols   = std.vector("int")(3)
for p in range(0,3):
	start_cols[p] = meta.col(start_wires[p])
	end_cols[p]   = meta.col(end_wires[p])
...
path = algo.findpath( ev_img.Image2DArray(), ev_badchs.Image2DArray(),start_row, end_row, start_cols, end_cols )
